...
double DiffChassis::slew(double target, bool is_left)
{
  ...
  double current = is_left ? this->prev_voltages.left : this->prev_voltages.right;

  if (fabs(current) > fabs(target)) step = 200;
...
bool DiffChassis::execute(DiffChassisCommand cmd, double max)
{
  return std::visit(
      overload{
          ...
          // Logic to brake one side of the drive allowing for a turn
          ...
          [this, max](diff_commands::Swing& v) {
            ...
            if (v.right == 0)
            {
              ...
              this->right_motors->brake();
              if (v_max > max) { v.left = v.left * max / v_max; }
              v.left = slew(v.left, true);
              this->left_motors->move_voltage(120 * v.left);
            ...
            else if (v.left == 0)
            {
              ...
              this->left_motors->brake();
              if (v_max > max) { v.right = v.right * max / v_max; }
              v.right = slew(v.right, false);
