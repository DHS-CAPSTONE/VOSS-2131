...
{
  while (task_running) { pros::delay(constants::MOTOR_UPDATE_DELAY); }
}

bool AbstractChassis::is_running() const { return task_running; }

void AbstractChassis::move_task(controller_ptr controller, ec_ptr ec, double max, voss::Flags flags)
{
  this->task = std::make_unique<pros::Task>([&, controller, ec, flags, max]() {
    ec->reset();
    controller->reset();
    while (!this->execute(
        controller->get_command(flags & voss::Flags::REVERSE, flags & voss::Flags::THRU, ec), max))
    {
  ...
  if (flags & voss::Flags::ASYNC) { return; }

  this->task->join();
}

void AbstractChassis::turn_task(
...
{
  this->task = std::make_unique<pros::Task>([&, controller, ec, flags, direction, max]() {
    ec->reset();
    controller->reset();
    while (!this->execute(
        controller->get_angular_command(
            flags & voss::Flags::REVERSE, flags & voss::Flags::THRU, direction, ec),
        max))
    {
      if (pros::competition::is_disabled())
      {
...
void AbstractChassis::move(
    double distance, controller_ptr controller, double max, voss::Flags flags)
{
  this->move(
      {distance, 0}, std::move(controller), this->default_ec, max, flags | voss::Flags::RELATIVE);
}

void AbstractChassis::move(
    double distance, controller_ptr controller, ec_ptr ec, double max, voss::Flags flags)
{
  this->move({distance, 0}, std::move(controller), std::move(ec), max, flags | Flags::RELATIVE);
}
...
void AbstractChassis::move(
    Pose target, controller_ptr controller, ec_ptr ec, double max, voss::Flags flags)
{
  while (this->task_running) { pros::delay(constants::MOTOR_UPDATE_DELAY); }
...
void AbstractChassis::turn(
    double target, double max, voss::Flags flags, voss::AngularDirection direction)
{
...
void AbstractChassis::turn_to(
    Point target, double max, voss::Flags flags, voss::AngularDirection direction)
{
    ...
    voss::AngularDirection direction)
{
  ...
  controller->set_target({target.x, target.y, std::nullopt}, flags & voss::Flags::RELATIVE, ec);
