...
void ExitConditions::set_target(voss::Pose new_target)
{
  ...
  for (auto ec : this->conditions) { ec->set_target(this->target_pose); }
}
...
ExitConditions& ExitConditions::add_tolerance(
    double linear_tolerance, double angular_tolerance, double tolerance_time)
{
  ...
  this->conditions.push_back(std::dynamic_pointer_cast<AbstractExitCondition>(ec));
  return *this;
}

ExitConditions& ExitConditions::add_thru_smoothness(double smoothness)
{
  this->conditions.push_back(std::make_shared<PrepLineExitCondition>(smoothness));
  return *this;
}

ExitConditions& ExitConditions::add_custom_condition(std::function<bool()> callback)
{
...
ExitConditions& ExitConditions::add_condition(std::shared_ptr<AbstractExitCondition> ec)
{
...
bool ExitConditions::is_met(voss::Pose current_pose, bool thru)
{
  ...
  {
    if (ec->is_met(current_pose, thru)) { return true; }
  }
...
bool ExitConditions::all_met(voss::Pose current_pose, bool thru)
{
  for (auto ec : this->conditions)
  {
    if (!ec->is_met(current_pose, thru)) { return false; }
  }
...
{
  for (auto ec : this->conditions) { ec->reset(); }
}
