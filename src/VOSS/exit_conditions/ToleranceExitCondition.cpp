...
bool ToleranceExitCondition::is_met(Pose pose, bool thru)
{
  ...
  else if (this->target_has_coordinate()) { exit = lin_exit->is_met(pose, thru); }
  else if (this->target_has_heading()) { exit = ang_exit->is_met(pose, thru); }
  if (get_debug() && exit) { printf("Tolerance Condition Met\n"); }
  return exit;
}
...
void ToleranceExitCondition::set_target(Pose target)
{
  ...
  if (this->ang_exit) { this->ang_exit->set_target(target); }
  if (this->lin_exit) { this->lin_exit->set_target(target); }
}

void ToleranceExitCondition::reset()
{
  if (this->ang_exit) { this->ang_exit->reset(); }
  if (this->lin_exit) { this->lin_exit->reset(); }
}
...
{
  this->lin_exit = std::make_shared<ToleranceLinearExitCondition>(linear_tolerance, tolerance_time);
}
}  // namespace voss::controller
