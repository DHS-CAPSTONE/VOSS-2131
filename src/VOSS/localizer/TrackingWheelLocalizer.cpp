...
TrackingWheelLocalizer::TrackingWheelLocalizer(
    ...
    double middle_dist)
    : AbstractLocalizer(),
      left_tracking_wheel(std::move(left)),
      right_tracking_wheel(std::move(right)),
      middle_tracking_wheel(std::move(middle)),
      imu(std::move(imu)),
      left_right_dist(left_right_dist),
...
void TrackingWheelLocalizer::update()
{
  ...
  {
    delta_left = left_tracking_wheel->get_dist_traveled() - prev_left_pos;
  }
  if (right_tracking_wheel)
  {
    delta_right = right_tracking_wheel->get_dist_traveled() - prev_right_pos;
  }
  if (middle_tracking_wheel)
  {
    delta_middle = middle_tracking_wheel->get_dist_traveled() - prev_middle_pos;
  }
  if (imu)
  {
  ...
  if (delta_angle)
  {
    ...
    else if (right_tracking_wheel) { local_x = (delta_right / delta_angle - left_right_dist) * i; }
    else if (left_tracking_wheel) { local_x = (delta_left / delta_angle + left_right_dist) * i; }
  ...
  this->pose.x =
      this->real_pose.x - (local_offset.x * cos(pose.theta) - local_offset.y * sin(pose.theta));
  this->pose.y =
      this->real_pose.y - (local_offset.x * sin(pose.theta) + local_offset.y * cos(pose.theta));
}

void TrackingWheelLocalizer::calibrate()
{
  if (left_tracking_wheel) { left_tracking_wheel->reset(); }
  if (right_tracking_wheel) { right_tracking_wheel->reset(); }
  if (middle_tracking_wheel) { middle_tracking_wheel->reset(); }
  if (imu)
  {
    imu->reset(true);
    while (imu->is_calibrating()) { pros::delay(constants::SENSOR_UPDATE_DELAY); }
  }
...
void TrackingWheelLocalizer::set_pose(Pose pose)
{
  ...
  this->real_pose.x =
      pose.x + (local_offset.x * cos(this->pose.theta) - local_offset.y * sin(this->pose.theta));
  this->real_pose.y =
      pose.y + (local_offset.x * sin(this->pose.theta) + local_offset.y * cos(this->pose.theta));
  this->prev_pose = this->real_pose;
  if (this->imu) { this->imu->set_rotation(-to_degrees(real_pose.theta)); }
}
