...
// Creating a localizer object with variable option based on adi input sensors
ADILocalizer::ADILocalizer(
    int left,
    int right,
    int mid,
    double lr_tpi,
      ...
      imu_ports(imu_port)
{
  ...
  if (left != 0)
    this->left_encoder = std::make_unique<pros::adi::Encoder>(abs(left), abs(left) + 1, left < 0);

  if (right != 0)
    this->right_encoder =
        std::make_unique<pros::adi::Encoder>(abs(right), abs(right) + 1, right < 0);

  if (mid != 0)
    this->middle_encoder = std::make_unique<pros::adi::Encoder>(abs(mid), abs(mid) + 1, mid < 0);
  if (imu_port != 0) this->imu = std::make_unique<pros::IMU>(imu_port);
}

double ADILocalizer::get_left_encoder_value()
{
  if (left_encoder) { return this->left_encoder->get_value(); }
  else { return 0.0; }
}

double ADILocalizer::get_right_encoder_value()
{
  if (right_encoder) { return this->right_encoder->get_value(); }
  else { return 0.0; }
}

double ADILocalizer::get_middle_encoder_value()
{
  if (middle_encoder) { return this->middle_encoder->get_value(); }
  else { return 0.0; }
}

double ADILocalizer::get_imu_value()
{
  if (imu) { return this->imu->get_rotation() * (M_PI / 180.0); }
  else
  {
    errno = EIO;
    return PROS_ERR;
...
{
  if (left_encoder) { this->left_encoder->reset(); }
  if (right_encoder) { this->right_encoder->reset(); }
  if (middle_encoder) { this->middle_encoder->reset(); }
  if (imu)
  {
    this->imu->reset();
    while (this->imu->is_calibrating()) { pros::delay(constants::SENSOR_UPDATE_DELAY); }
  }
...
// in the left and right encoder values Angle is the difference between the robot
...
void ADILocalizer::update()
{
  ...
  if (right_encoder) delta_right = (right_pos - prev_right_pos) / left_right_tpi;

  double delta_middle = 0.0;
  if (middle_encoder) delta_middle = (middle_pos - prev_middle_pos) / middle_tpi;
...
void ADILocalizer::set_pose(Pose pose)
{
  ...
  if (this->imu && pose.theta.has_value()) { this->imu->set_rotation(-pose.theta.value()); }
}
void ADILocalizer::set_pose(double x, double y, double theta) { this->set_pose({x, y, theta}); }
