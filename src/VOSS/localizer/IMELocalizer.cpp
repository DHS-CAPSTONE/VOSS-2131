...
// Creating a localizer object with variable option based on internal motor
// encoders
IMELocalizer::IMELocalizer(
    std::vector<int8_t> left_motors_ports,
      ...
      imu_ports(imu_port)
{
  ...
  {
    this->horizontal_motors = std::make_unique<pros::MotorGroup>(horizontal_motors_ports);
  }
...
{
  if (left_motors) { return this->left_motors->get_position(); }
  else
  {
    errno = EIO;
    return PROS_ERR;
...
{
  if (right_motors) { return this->right_motors->get_position(); }
  else
  {
    errno = EIO;
    return PROS_ERR;
...
{
  if (horizontal_motors) { return this->horizontal_motors->get_position(); }
  else
  {
    errno = EIO;
    return PROS_ERR;
...
void IMELocalizer::calibrate()
{
  ...
  if (left_motors) { left_motors->tare_position(); }
  if (right_motors) { right_motors->tare_position(); }
  if (horizontal_motors) { horizontal_motors->tare_position(); }
...
void IMELocalizer::update()
{
  ...
  if (right_motors) delta_right = (right_pos - prev_right_pos) / left_right_tpi;
...
void IMELocalizer::set_pose(double x, double y, double theta) { this->set_pose({x, y, theta}); }
