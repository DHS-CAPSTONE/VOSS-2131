...
TrackingWheelLocalizerBuilder::TrackingWheelLocalizerBuilder()
    : left_right_dist(0.0),
      middle_dist(0.0),
      left_tracking_wheel(nullptr),
      right_tracking_wheel(nullptr),
      middle_tracking_wheel(nullptr),
      imu(nullptr)
{
}
...
TrackingWheelLocalizerBuilder& TrackingWheelLocalizerBuilder::with_left_encoder(
    int smart_port, int adi_port)
{
...
TrackingWheelLocalizerBuilder& TrackingWheelLocalizerBuilder::with_right_encoder(
    int smart_port, int adi_port)
{
...
TrackingWheelLocalizerBuilder& TrackingWheelLocalizerBuilder::with_middle_encoder(
    int smart_port, int adi_port)
{
...
{
  if (left_tracking_wheel) { left_tracking_wheel->set_tpi(tpi); }
  if (right_tracking_wheel) { right_tracking_wheel->set_tpi(tpi); }
  return *this;
}
...
std::shared_ptr<TrackingWheelLocalizer> TrackingWheelLocalizerBuilder::build()
{
  return std::make_shared<TrackingWheelLocalizer>(
      std::move(left_tracking_wheel),
      std::move(right_tracking_wheel),
      std::move(middle_tracking_wheel),
      std::move(imu),
      left_right_dist,
      middle_dist);
}
