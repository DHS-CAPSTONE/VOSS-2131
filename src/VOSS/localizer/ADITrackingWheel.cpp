...
{
  this->encoder =
      std::make_unique<pros::adi::Encoder>(abs(adi_port), abs(adi_port) + 1, adi_port < 0);
}
...
double ADITrackingWheel::get_raw_position() { return this->encoder->get_value(); }

void ADITrackingWheel::reset() { this->encoder->reset(); }
