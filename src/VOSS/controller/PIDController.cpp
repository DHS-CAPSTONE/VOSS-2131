...
chassis::DiffChassisCommand PIDController::get_command(
    bool reverse, bool thru, std::shared_ptr<AbstractExitCondition> ec)
{
  ...
  if (distance_error < min_error)
  {
    ...
    if (noPose)
    {
      switch (min_err_behavior)
      {
        ...
        case MinErrBehavior::MAINTAIN_HEADING: {
          if (std::isnan(min_err_heading)) { min_err_heading = current_angle; }
    ...
    else
    {
      ...
      while (fabs(poseError) > M_PI) poseError -= 2 * M_PI * poseError / fabs(poseError);
  ...
  {
    printf(
        "dist_err: %.2f, ang_err: %.2f, lin_speed: %.2f, ang_speed: %.2f\n",
        distance_error,
        angle_error,
        lin_speed,
        ang_speed);
    ...
    std::shared_ptr<AbstractExitCondition> ec)
{
  ...
  else { target_angle = this->angular_target; }
  ...
  if (ec->is_met(this->l->get_pose(), thru) || chainedExecutable)
  {
    if (thru)
    {
      return chassis::DiffChassisCommand{chassis::diff_commands::Chained{-ang_speed, ang_speed}};
    }
    return chassis::Stop{};
  }
  return chassis::DiffChassisCommand{chassis::diff_commands::Voltages{-ang_speed, ang_speed}};
}
...
std::shared_ptr<PIDController> PIDController::modify_linear_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = PIDControllerBuilder::from(*this).with_linear_constants(kP, kI, kD).build();
...
std::shared_ptr<PIDController> PIDController::modify_angular_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = PIDControllerBuilder::from(*this).with_angular_constants(kP, kI, kD).build();
...
std::shared_ptr<PIDController> PIDController::modify_min_error(double min_error)
{
  auto pid_mod = PIDControllerBuilder::from(*this).with_min_error(min_error).build();
...
void PIDController::set_overturn(bool overturn) { enable_overturn = overturn; }
