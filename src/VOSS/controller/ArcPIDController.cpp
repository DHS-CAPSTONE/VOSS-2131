...
ArcPIDController::ArcPIDController(std::shared_ptr<localizer::AbstractLocalizer> l)
    : AbstractController(std::move(l))
{
}

chassis::DiffChassisCommand ArcPIDController::get_command(
    bool reverse, bool thru, std::shared_ptr<AbstractExitCondition> ec)
{
  ...
  double t;
  if (fabs(a * e - d * b) < 0.00001) { t = 0.0; }
  else { t = (c * e - f * b) / (a * e - d * b); }
  ...
  if (reverse) { angle_error = atan2(-b, e) - current_angle; }
  ...
  else if (fabs(angle_error) > M_PI_2 && this->can_reverse) { lin_speed = -lin_speed; }

  if (lin_speed > prev_lin_speed + slew) { lin_speed = prev_lin_speed + slew; }
  ...
  if (arc_radius != 0.0)
  {
    ...
    if ((bool)(arc_radius > 0) != reverse) { tangent += M_PI_2; }
    else { tangent -= M_PI_2; }
  ...
  return chassis::DiffChassisCommand{chassis::diff_commands::Voltages{left_speed, right_speed}};
}
...
std::shared_ptr<ArcPIDController> ArcPIDController::modify_linear_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = ArcPIDControllerBuilder::from(*this).with_linear_constants(kP, kI, kD).build();
...
std::shared_ptr<ArcPIDController> ArcPIDController::modify_track_width(double track_width)
{
  auto pid_mod = ArcPIDControllerBuilder::from(*this).with_track_width(track_width).build();
...
std::shared_ptr<ArcPIDController> ArcPIDController::modify_min_error(double min_error)
{
  auto pid_mod = ArcPIDControllerBuilder::from(*this).with_min_error(min_error).build();
