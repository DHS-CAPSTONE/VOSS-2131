...
AbstractController::AbstractController(std::shared_ptr<localizer::AbstractLocalizer> l)
{
  this->l = l;
}
...
void AbstractController::set_target(
    Pose target, bool relative, std::shared_ptr<AbstractExitCondition> ec)
{
  if (target.theta.has_value()) { target.theta = voss::to_radians(*target.theta); }
  if (relative)
  {
    ...
    if (target.theta.has_value()) { this->target = Pose{x_new, y_new, target.theta.value() + h}; }
    else { this->target = Pose{x_new, y_new, std::nullopt}; }
  }
...
void AbstractController::set_angular_target(double angular_target, bool relative)
{
  ...
  else { this->angular_target = voss::norm(angular_target); }
}
