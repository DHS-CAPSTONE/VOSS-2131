...
chassis::DiffChassisCommand SwingController::get_command(
    ...
    std::shared_ptr<AbstractExitCondition> ec)
{
  ...
  // Power applied to motors based on proportion of the error and the weight of
  ...
  if (!((ang_speed >= 0.0) ^ (this->prev_ang_speed < 0.0)) && this->prev_ang_speed != 0)
  {
...
std::shared_ptr<SwingController> SwingController::modify_angular_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = SwingControllerBuilder::from(*this).with_angular_constants(kP, kI, kD).build();
