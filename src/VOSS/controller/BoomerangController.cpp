...
{
  if (!target.theta.has_value()) { return chassis::DiffChassisCommand{chassis::Stop{}}; }
  ...
  {
    carrot_point = {
        target.x - distance_error * cos(target_angle) * lead_pct,
  ...
  double angle_error;
  if (!reverse) { angle_error = atan2(dy, dx) - current_angle; }
  else { angle_error = atan2(-dy, -dx) - current_angle; }
  ...
  if (thru)
  {
    if (thru_behavior == ThruBehavior::FULL_SPEED) { lin_speed = 100; }
    else { lin_speed = fmax(lin_speed, min_vel); }
  }
  ...
  if (distance_error < min_error)
  {
    ...
    else
    {
      ...
      double scaled_angle_error =
          norm_delta(scale_factor * angle_error + (1 - scale_factor) * pose_error);
  ...
  else
  {
    if (fabs(angle_error) > M_PI_2 && this->can_reverse)
    {
      ...
      if (cosine_scaling != CosineScaling::ALL_THE_TIME) { lin_speed = -lin_speed; }
    }
  ...
  {
    printf(
        "carrot point: %.2f, %.2f, dist_err: %.2f, ang_err: %.2f, lin_speed: %.2f, ang_speed: "
        "%.2f\n",
        carrot_point.x,
        carrot_point.y,
        distance_error,
...
std::shared_ptr<BoomerangController> BoomerangController::modify_linear_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = BoomerangControllerBuilder::from(*this).with_linear_constants(kP, kI, kD).build();
...
std::shared_ptr<BoomerangController> BoomerangController::modify_angular_constants(
    double kP, double kI, double kD)
{
  auto pid_mod = BoomerangControllerBuilder::from(*this).with_angular_constants(kP, kI, kD).build();
...
std::shared_ptr<BoomerangController> BoomerangController::modify_min_error(double min_error)
{
  auto pid_mod = BoomerangControllerBuilder::from(*this).with_min_error(min_error).build();
...
std::shared_ptr<BoomerangController> BoomerangController::modify_lead_pct(double lead_pct)
{
  auto pid_mod = BoomerangControllerBuilder::from(*this).with_lead_pct(lead_pct).build();
...
void BoomerangController::set_overturn(bool overturn) { enable_overturn = overturn; }
