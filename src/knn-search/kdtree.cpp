...
class DistanceL2 : virtual public DistanceMeasure
{
 ...
 public:
  ...
  double distance(const CoordPoint& p, const CoordPoint& q)
  {
    ...
    if (w)
    {
      for (i = 0; i < p.size(); i++) dist += (*w)[i] * (p[i] - q[i]) * (p[i] - q[i]);
    }
    else
    {
...
void KdTree::set_distance(int distance_type, const DoubleVector* weights /*=NULL*/)
{
  ...
  if (distance_type == 0) { distance = (DistanceMeasure*)new DistanceL0(weights); }
  else if (distance_type == 1) { distance = (DistanceMeasure*)new DistanceL1(weights); }
  else { distance = (DistanceMeasure*)new DistanceL2(weights); }
}
...
//--------------------------------------------------------------
void KdTree::k_nearest_neighbors(
    const CoordPoint& point, size_t k, KdNodeVector* result, KdNodePredicate* pred /*=NULL*/)
{
  size_t i;
  KdNode temp;
...
//--------------------------------------------------------------
bool KdTree::neighbor_search(
    const CoordPoint& point, kdtree_node* node, size_t k, SearchQueue* neighborheap)
{
  double curdist, dist;
  ...
  {
    if (neighborheap->size() < k) { neighborheap->push(nn4heap(node->dataindex, curdist)); }
...
//--------------------------------------------------------------
void KdTree::range_search(
    const CoordPoint& point, kdtree_node* node, double r, std::vector<size_t>* range_result)
{
